...
it('should return no offers', async () => {
  const hobbies = []
  mockAxios.get.mockResolvedValueOnce(hobbies);
  
  const result = await OffersDataService();
 
  expect(mockAxios.get).toHaveBeenCalledTimes(1);
  expect(mockAxios.get).toHaveBeenCalledWith(`http://localhost:8080/business-owner/`);
...
it('should render multiple offers', async () => {
  const hobbies = [
    {
