...
beforeEach(() => render(<Router>  <CreateOffer /></Router>));
afterEach(() => {
  mockAxios.reset();
  cleanup;
});
...
it('should show error message on invalid input', () => {
  ...
  const errorName = screen.getByText(/Hobby name must be at least 3 characters long/i);
  const errorSlogan = screen.getByText(/Text has to be between 2 and 20 characters long/i);
...
it('should create offer successfully', async () => {
  const info = {
    ...
    name: "Climbing",
    price: 73,
    profileImgUrl: "https://res.cloudinary.com/dv6ktrxwv/image/upload/v1640706607/i5gfpc0gesnyp4whkvul.jpg",
    profileImg_id: null,
    slogan: null
  };
  ...
  expect(mockAxios.post).toHaveBeenCalledWith(`http://localhost:8080/hobbies/create-offer`, info);
