...
it('should authenticate user creadentials successfully', async () => {
  ...
  expect(mockAxios.post).toHaveBeenCalledWith(`http://localhost:8080/users/authenticate`,
      {
        "password": "123",
        "username": "user",
      }
...
it('should assign correct user role successfully', async () => {
  ...
  expect(mockAxios.post).toHaveBeenCalledTimes(1);
  expect(mockAxios.post).toHaveBeenCalledWith(`http://localhost:8080/users/login/user`);
  expect(result).toEqual("USER");
