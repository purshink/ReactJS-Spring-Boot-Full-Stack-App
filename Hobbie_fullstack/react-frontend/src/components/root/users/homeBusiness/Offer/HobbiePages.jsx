...
const HobbiePages = () => {
    ...
    return (
        <>
            {isColumnBasedSmall && <div> <span className={styles.hobbie_title_small}><b>{hobby.name}</b>

            </span> <h4 className={styles.slogan_small}> {hobby.slogan} </h4></div>}
            ...
            <section className={isColumnBasedSmall ? styles.hobbie_container_small : styles.hobbie_container}>
                {hobby !== undefined && <div className={isColumnBasedSmall ? styles.hobbie_content_small : styles.hobbie_content}>
                    ...
                    {currentPage !== '03' && <section className={styles.hobbie_cover}  >
                        <img className={styles.hobbie_cover} src={hobby.profileImgUrl} alt="profile" /></section>}

                    <div className={isColumnBasedSmall ? styles.hobbie_content_body_small : styles.hobbie_content_body}>

                        {!isColumnBasedSmall && <article className={styles.hobbie_pages}>
                        ...
                        {isColumnBasedSmall && <article className={styles.hobbie_pages_horizontal}>
                        ...
                        <section className={styles.hobbie_lable}>
                            {!isColumnBasedSmall && <div> <span className={styles.hobbie_title}><b>{hobby.name}</b></span> 
                            <h4 className={styles.slogan}> {hobby.slogan} </h4></div>}
                            ...
                            {currentPage !== '03' && currentPage !== '04' && <article className={styles.buttons}>
                                {isBusinessLoggedIn &&
                                    <div> 
                                        <Link to="#" onClick={handleEdit(hobby)} className={styles.btn}>Edit </Link>
                                        <Link to="#" onClick={handleDelete(hobby)} className={styles.btn}>Delete </Link>
                                    </div>}
                                {isUserLoggedIn && <div onClick={handleSave(hobby.id)}>
                                    {saved ? <span className={styles.btn}>Remove</span> 
